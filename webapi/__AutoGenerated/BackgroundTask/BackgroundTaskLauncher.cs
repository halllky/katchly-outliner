...
namespace FlexTree {
    public sealed class BackgroundTaskLauncher : Microsoft.Extensions.Hosting.BackgroundService {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken) {
            ...
            FlexTree.DefaultConfigurer.InitAsBatchProces(serviceCollection);
            ...
            try {
                ...
                while (!stoppingToken.IsCancellationRequested) {
                    ...
                    var dbContext = pollingScope.ServiceProvider.GetRequiredService<FlexTree.MyDbContext>();
            ...
            try {
                ...
                var dbContext = disposingScope.ServiceProvider.GetRequiredService<FlexTree.MyDbContext>();
                DetectFinishing(runningTasks, dbContext, logger);
            } catch (Exception ex) {
        ...
        /// </summary>
        private static JobChain CreateExecuteArgument(BackgroundTask backgroundTask, FlexTree.BackgroundTaskEntity entity, BackgroundTaskContextFactory contextFactory, CancellationToken stoppingToken) {
        ...
        /// </summary>
        private void DetectFinishing(Dictionary<string, Task> runningTasks, FlexTree.MyDbContext dbContext, ILogger logger) {
            // 終了したバッチを列挙
...
namespace FlexTree {
    ...
    public class BackgroundTaskContext : IDisposable {
        ...
        public FlexTree.MyDbContext DbContext => ServiceProvider.GetRequiredService<FlexTree.MyDbContext>();
...
namespace FlexTree {
    partial class MyDbContext {
        public virtual DbSet<FlexTree.BackgroundTaskEntity> NIJOBackgroundTaskEntityDbSet { get; set; }
    }
}
