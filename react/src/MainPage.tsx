...
import { ArrowPathIcon, ChevronRightIcon, ChatBubbleLeftEllipsisIcon, Cog6ToothIcon, InboxIcon, PlusIcon, CubeIcon, MinusIcon } from '@heroicons/react/24/outline'
import { Panel, PanelGroup, PanelResizeHandle } from 'react-resizable-panels'
import * as Util from './__autoGenerated/util'
import * as Input from './__autoGenerated/input'
import * as Collection from './__autoGenerated/collection'
import { Comment, GridRow, GridRowOfRowObject, PageFormState, ROWTYPE_STYLE, RowObject, RowType, RowTypeId, RowTypeMapDispatcher, UseFieldArrayReturnType, createNewComment, createNewRowType, getAboveRowObjectIndex, getAttrCellValue, getBelowRowObjectIndex, getLabelCellValue, getRowEditState, insertNewRow, moveArrayItem, updateIndentInfo, setAttrCellValue, setLabelCellValue, toGridRows, useEditRowObject, useRowTypeMap } from './Types'
      ...
      cell: cellProps => {
        ...
        return (
          <div className="flex">
            ...
            <div className={`flex-1 flex ${bgColor}`}>
...
const Indent = ({ row, indentSize }: {
  ...
  // // 不具合発生時のデバッグはここでこのログを出すと分かりやすい
  // console.log(getLabelCellValue(row, new Map()), JSON.stringify(row.indentInfo.map(a => a.hasVerticalLine)))
  return <>
    {row.indentInfo.map(({ hasVerticalLine }, i) => (
      <div
        key={i}
        className={hasVerticalLine ? 'bg-color-2 border-x border-color-3' : ''}
        style={{ flexBasis: indentSize }}
      ></div>
    ))}
  </>
}
