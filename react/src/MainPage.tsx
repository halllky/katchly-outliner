...
import { GridRow, GridRowOfRowObject, GridRowOfRowType, PageFormState, ROWTYPE_STYLE, RowObject, RowType, RowTypeId, UseFieldArrayReturnType, getAboveRowObjectIndex, getAttrCellValue, getBelowRowObjectIndex, getLabelCellValue, getRowEditState, insertNewRow, moveArrayItem, rowTypeMapReducer, setAttrCellValue, setLabelCellValue, toGridRows, useRecalculateGridRow } from './Types'
...
const AfterLoaded = ({ rowData, rowTypeData, onSave, className, style, children }: {
  ...
  const { fields, insert, update, remove, move } = useFieldArray({ control, name: 'gridRows' })
  useEffect(() => {
  ...
  // -------------------------------------
  // 更新
  const recalculate = useRecalculateGridRow(insert, update, remove)
  const changeRowType: UseFieldArrayReturnType['update'] = useCallback((index, row) => {
    update(index, row)

    const rowInFields = fields[index] as GridRowOfRowObject // 古い状態をもとに再計算されてしまうのを防ぐ
    rowInFields.item.type = (row as GridRowOfRowObject).item.type
    recalculate([index, index], fields)
  ...
  const onKeyDown: React.KeyboardEventHandler = useCallback(e => {
    ...
    // Alt + 上下 による行移動
      ...
      if (e.key === 'ArrowUp') {
        ...
        move(below, first)
        recalculate([below, first], moveArrayItem(fields, below, first))
      }
    }
  }, [update, move, toggleSideMenu, handleAddRow, handleDeleteRows, onSave, fields, rowTypeMap, recalculate])
