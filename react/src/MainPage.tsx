...
import { GridRow, GridRowOfRowObject, PageFormState, ROWTYPE_STYLE, RowObject, RowType, RowTypeId, RowTypeMapDispatcher, UseFieldArrayReturnType, countComment, createNewThread, getAboveRowObjectIndex, getAttrCellValue, getBelowRowObjectIndex, getLabelCellValue, getRowEditState, insertNewRow, moveArrayItem, setAttrCellValue, setLabelCellValue, toGridRows, useEditRowObject, useRowTypeMap } from './Types'
...
const ThreadView = ({ row, rowIndex, onChange, rowTypeMap, dispatchRowType, className }: {
  ...
  const appendNewThread = useCallback(() => {
    if (!onChange || !newItemText || !userName || !row || !rowIndex) return
    const newThread = createNewThread(userName)
    newThread.text = newItemText
    setNewItemText('')
    if (row.type === 'row') {
  ...
  }, [row, rowIndex, rowTypeMap, newItemText, setNewItemText, userName, onChange, dispatchRowType])
  ...
  return (
    <div className={`flex flex-col gap-1 ${className ?? ''}`}>
      ...
      <div className="flex flex-col">
        ...
        />
        <Input.IconButton icon={PaperAirplaneIcon} className="self-end p-3" hideText onClick={appendNewThread}>
          コメント追加
        </Input.IconButton>
      </div>
