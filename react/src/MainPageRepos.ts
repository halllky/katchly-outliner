...
export const useKatchlyRepository = () => {
  ...
  const saveAll = useCallback(async ({ rows, rowTypes }: { rows: RowObject[], rowTypes: RowType[] }): Promise<void> => {
    ...
    try {
      ...
      // サーバーAPI型に変換する
      const rowObjectSaveCommands = rows.map(rowObject => {
        const [state, row] = RowObjectConverter.toServerApiType(rowObject)
        return { state, row }
      })
      ...
      // RowOrder: delete & insert (Row削除前に消さないと外部キー制約に引っかかる)
      await post(`/api/RowOrder/delete-all-row-order`, {})
      ...
      // Row: create, update
      for (const { state, row } of rowObjectSaveCommands.filter(x => x.state === '+' || x.state === '*')) {
        if (state === '+') {
      ...
      // Row: delete
      for (const { row } of rowObjectSaveCommands.filter(x => x.state === '-')) {
      ...
      // RowOrder: delete & insert (全件INSERT)
      const RowObjectIdList = rowObjectSaveCommands
        .filter(cmd => cmd.state !== '-')
        .map(cmd => cmd.row.ID ?? '')
        .join('\n')
