...
const Page = () => {
  ...
  const { load } = Util.use参照先Repository(pkArray)
...
const AfterLoaded = ({
  ...
  const navigateToEditView = useCallback((e: React.MouseEvent) => {
    navigate(`/x0980ef37494eb0089d5695ded11e38fa/edit/${window.encodeURI(`${pkArray[0]}`)}`)
    e.preventDefault()
  }, [navigate, pkArray])
    ...
    <>
      <VForm.Container leftColumnMinWidth="10.4rem">
        ...
        <VForm.Item label="Name">
          <Input.Word {...registerEx(`own_members.Name`)} readOnly />
        </VForm.Item>
      </VForm.Container>
    </>
  )
