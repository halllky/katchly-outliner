...
import { ArrowPathIcon, ChatBubbleLeftEllipsisIcon, Cog6ToothIcon, InboxIcon, PlusIcon, CubeIcon, MinusIcon, ArrowDownTrayIcon } from '@heroicons/react/24/outline'
...
const AfterLoaded = ({ rowData, rowTypeData, onSave, nowSaving, className, style, children }: {
  ...
  // -------------------------------------
  // ダンプ
  const { download } = Util.useHttpRequest()
  const [nowDumping, setNowDumping] = useState(false)
  const handleExcelDump = useCallback(async () => {
    if (nowDumping) return
    try {
      setNowDumping(true)
      await download('/api/Row/dump-excel', 'かっちり.xlsx')
    } finally {
      setNowDumping(false)
    }
  }, [nowDumping, download])
      ...
      >
        <Panel className="flex flex-col">
          <div className="flex gap-1 items-center px-px h-main-page-header border-b border-color-3">
            ...
            <div className="basis-44">
              <Input.IconButton icon={ArrowDownTrayIcon} onClick={handleExcelDump} className="p-1" outline>
                {nowDumping ? 'エクスポート中...' : 'エクスポート'}
              </Input.IconButton>
            </div>
