...
export type DetailViewRef = {
  focus: () => void
}

export const DetailView = Util.forwardRefEx<DetailViewRef, DetailViewProp>(({ row, rowIndex, rowTypeMap, changeRowType, updateRow, dispatchRowType, onEscape }, ref) => {
...
const CommentList = Util.forwardRefEx<CommentListRef, CommentListProps>(({ row, rowIndex, onChange, rowTypeMap, dispatchRowType, onEscape }, ref) => {
  ...
  }, [gridRef, userName, editCommentArray, dispatchMsg, onEscape])
      ...
      if (userName) {
        editCommentArray(comments => {
          if (comments.length === 0) {
            setTimeout(() => gridRef.current?.startEditing(), 10)
            return [...comments, createNewComment(userName)]
          } else {
            return comments
          }
        })
  ...
  }), [gridRef, userName, editCommentArray])
